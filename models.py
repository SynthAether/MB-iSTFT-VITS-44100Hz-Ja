...
class Multiband_iSTFT_Generator(torch.nn.Module):
    ...
    def forward(self, x, g=None):
      stft = TorchSTFT(filter_length=self.gen_istft_n_fft, hop_length=self.gen_istft_hop_size, win_length=self.gen_istft_n_fft).to(x.device)  # [B, 192,32]
      pqmf = PQMF(x.device)
...
class SynthesizerTrn(nn.Module):
  ...
  def forward(self, x, x_lengths, y, y_lengths, sid=None):
    ...
    z_slice, ids_slice = commons.rand_slice_segments(z, y_lengths, int(self.segment_size))
    #z_slice, ids_slice = commons.rand_slice_segments(z, y_lengths, self.segment_size)

    z_slice_last, ids_slice_last = commons.last_slice_segments(z, y_lengths, int(self.segment_size))

    o, o_mb = self.dec(z_slice, g=g)
    o_last, o_mb_last = self.dec(z_slice_last, g=g)
    return o, o_mb, l_length, attn, ids_slice, x_mask, y_mask, (z, z_p, m_p, logs_p, m_q, logs_q), o_last, ids_slice_last
